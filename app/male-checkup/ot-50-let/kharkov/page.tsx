...
export const metadata: Metadata = {
  title: 'Чекап для мужчин от 50 лет в Харькове 2026 | check-up.in.ua',
  description: 'Комплексное обследование для мужчин от 50 лет в Харькове: сердце, сосуды, простата, онкология. ОН Клиник Харьков.',
  alternates: {
    canonical: 'https://check-up.in.ua/male-checkup/ot-50-let/kharkov',
    languages: {
      'ru': 'https://check-up.in.ua/male-checkup/ot-50-let/kharkov',
      'uk': 'https://check-up.in.ua/ukr/male-checkup/vid-50-rokiv/kharkiv',
    },
  },
};
...
export default async function Page() {
  const { data: programs } = await db()
    ...
    .eq('clinic_id', CLINIC_ID).eq('gender', 'male')
    .in('age_group', ['50+']).eq('is_specialized', false)
    ...
    <main className="bg-[#f9fbfd] min-h-screen">
      <div className="max-w-4xl mx-auto px-4 py-8">
        <nav className="text-xs text-gray-400 mb-6 flex flex-wrap gap-1 items-center">
          ...
          <Link href="/male-checkup/kharkov" className="hover:text-[#005485]">Чекап для мужчин</Link><span>→</span>
          <span className="text-gray-600">От 50 лет</span>
        </nav>
        <div className="mb-10">
          <h1 className="text-3xl font-bold text-[#0b1a24] mb-3 h1-teal-line">Чекап для мужчин от 50 лет в Харькове</h1>
          <p className="text-gray-500 mt-4 max-w-xl">Комплексное обследование для мужчин от 50 лет в Харькове: сердце, сосуды, простата, онкология. ОН Клиник Харьков.</p>
        </div>
        {safe.length > 0 ? (
        ...
        <section className="bg-white rounded-[12px] border border-gray-100 px-6 py-6 mb-10 text-sm text-gray-600 space-y-3">
          <p>После 50 лет мужской чекап охватывает расширенную кардиологическую оценку, скрининг рака простаты (ПСА + УЗИ), анализ на онкомаркеры и денситометрию.</p>
          <p>Регулярность обследования — раз в 6 месяцев — позволяет отслеживать динамику показателей и своевременно корректировать лечение.</p>
        </section>
