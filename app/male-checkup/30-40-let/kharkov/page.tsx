...
export const metadata: Metadata = {
  title: 'Чекап для мужчин 30–40 лет в Харькове 2026 | check-up.in.ua',
  description: 'Комплексное обследование для мужчин 30–40 лет в Харькове: сердце, метаболизм, тестостерон, простата. ОН Клиник.',
  alternates: {
    canonical: 'https://check-up.in.ua/male-checkup/30-40-let/kharkov',
    languages: {
      'ru': 'https://check-up.in.ua/male-checkup/30-40-let/kharkov',
      'uk': 'https://check-up.in.ua/ukr/male-checkup/30-40-rokiv/kharkiv',
    },
  },
};
...
export default async function Page() {
  const { data: programs } = await db()
    ...
    .in('age_group', ['30-40']).eq('is_specialized', false)
    ...
    <main className="bg-[#f9fbfd] min-h-screen">
      <div className="max-w-4xl mx-auto px-4 py-8">
        <nav className="text-xs text-gray-400 mb-6 flex flex-wrap gap-1 items-center">
          ...
          <span className="text-gray-600">30–40 лет</span>
        </nav>
        <div className="mb-10">
          <h1 className="text-3xl font-bold text-[#0b1a24] mb-3 h1-teal-line">Чекап для мужчин 30–40 лет в Харькове</h1>
          <p className="text-gray-500 mt-4 max-w-xl">Комплексное обследование для мужчин 30–40 лет в Харькове: сердце, метаболизм, тестостерон, простата. ОН Клиник.</p>
        </div>
        {safe.length > 0 ? (
        ...
        <section className="bg-white rounded-[12px] border border-gray-100 px-6 py-6 mb-10 text-sm text-gray-600 space-y-3">
          <p>В 30–40 лет возрастают риски сердечно-сосудистых заболеваний и метаболического синдрома. Программа включает липидный профиль, ЭКГ, уровень тестостерона и ПСА.</p>
          <p>Раннее выявление гипертонии, преддиабета и гормональных отклонений значительно снижает риски в будущем.</p>
        </section>
