...
export const metadata: Metadata = {
  title: 'Чекап для женщин в Харькове 2026 — программы и цены | check-up.in.ua',
  description: 'Комплексное обследование для женщин в Харькове: программы по возрасту от 18 до 60+. Состав анализов, цены, онлайн-запись. ОН Клиник.',
  alternates: {
...
const AGE_NAV = [
  { label: 'До 30 лет', href: '/female-checkup/do-30-let/kharkov' },
  { label: '30–40 лет', href: '/female-checkup/30-40-let/kharkov' },
  { label: '40–50 лет', href: '/female-checkup/40-50-let/kharkov' },
  { label: 'От 50 лет', href: '/female-checkup/ot-50-let/kharkov' },
];

const FAQ = [
  { q: 'Что входит в женский чекап?', a: 'В зависимости от программы: ОАК, гормоны (ТТГ, эстрадиол), онкомаркеры (CA 125, CA 15-3), УЗИ малого таза и молочных желёз, консультация гинеколога.' },
  { q: 'Как часто нужен профилактический чекап?', a: 'До 40 лет — раз в год. После 40 — раз в 6 месяцев или по рекомендации врача.' },
  { q: 'Можно ли сдавать анализы во время менструации?', a: 'Большинство анализов крови и УЗИ — да. Для гормонального профиля и мазков лучше планировать на 3–7 или 14–21 день цикла.' },
  { q: 'Как подготовиться к обследованию?', a: 'Прийти натощак (8–12 часов без еды), не употреблять алкоголь 24 часа, не курить 2 часа до сдачи крови.' },
];
...
export default async function Page() {
  const { data: programs } = await db()
    ...
    .eq('clinic_id', CLINIC_ID).eq('gender', 'female').eq('is_specialized', false)
  ...
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{__html: JSON.stringify({
        ...
        name: 'Чекап для женщин в Харькове', url: 'https://check-up.in.ua/female-checkup/kharkov', inLanguage: 'ru',
      })}} />
      <main className="bg-[#f9fbfd] min-h-screen">
        <div className="max-w-4xl mx-auto px-4 py-8">
          <nav className="text-xs text-gray-400 mb-6 flex flex-wrap gap-1 items-center">
            ...
            <span className="text-gray-600">Чекап для женщин</span>
          </nav>
          <div className="mb-10">
            <h1 className="text-3xl font-bold text-[#0b1a24] mb-3 h1-teal-line">Женский чекап в Харькове</h1>
            <p className="text-gray-500 mt-4 mb-4 max-w-xl">Комплексные программы обследования для женщин с учётом возрастных рисков — от базового скрининга до расширенного гормонального и онкологического профиля.</p>
            <div className="flex flex-wrap gap-4 text-sm text-gray-500">
              <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-[#04D3D9] inline-block" />{safe.length} программ для женщин</span>
          ...
          <section className="mb-12">
            <h2 className="text-2xl font-bold text-[#0b1a24] mb-6">Все программы женского чекапа</h2>
          ...
          <section className="bg-white rounded-[12px] border border-gray-100 px-6 py-7 mb-10">
            <h2 className="text-xl font-bold text-[#0b1a24] mb-4">Зачем женщине регулярный чекап</h2>
            <div className="space-y-3 text-sm text-gray-600">
              <p>Комплексное обследование для женщин охватывает основные риски: сердечно-сосудистые заболевания, нарушения гормонального фона, щитовидная железа и онкологический скрининг.</p>
              <p>В Харькове программы женского чекапа доступны в сети ОН Клиник — современной клинике с собственной лабораторией и узкими специалистами под одной крышей.</p>
              <p>Программы подобраны по возрастным потребностям: до 30 — базовый скрининг и репродуктивное здоровье; 30–40 — гормональный профиль; после 40 — расширенный онко- и кардиологический мониторинг.</p>
            </div>
          </section>
          <section className="mb-10">
