...
// Контент: content/kharkiv/male-do-30-rokiv.md (v0) дослівно; файл згенеровано з того самого джерела, що й MD.
...
const PAGE_PATH = '/ukr/male-checkup/do-30-rokiv/kharkiv';
const PAGE_URL = `https://check-up.in.ua${PAGE_PATH}`;
const PLATFORM_PROGRAM = 'male-checkup-do-30';
const CLINIC_SLUG = 'onclinic-kharkiv';
const SOURCE_CTA = 'age_page_male_do_30_kharkiv';
// SEO-STANDARD р.4, Тип 5a. X (мінімальна ціна програм клініки для сторінки) – з Supabase у generateMetadata.
const TITLE = "Чекап для чоловіків до 30 років: які обстеження проходити, програми в Харкові | check-up.in.ua";
const DESCRIPTION_BASE = "Які обстеження потрібні чоловікам до 30 років. 3 цілі скринінгу.";
...
const FAQ: { q: string; a: string }[] = [
  { q: "Мені до 30 і нічого не турбує. Навіщо обстеження?", a: "Щоб мати точку відліку і не пропустити того, що в цьому віці шукають за настановами: підвищений тиск і рівень холестерину. Перелік і частота – у блоці «Що вам потрібно в цьому віці»." },
...
export default async function MaleAgeDo30KharkivPage() {
  ...
  const jsonLd: object[] = [
    {
      ...
      '@type': 'MedicalWebPage',
      name: "Чекап для чоловіків до 30 років – що перевіряти і де пройти в Харкові",
      url: PAGE_URL,
    ...
    <>
      <main className="text-[#0b1a24] pb-24 md:pb-0">
        ...
        <section style={{ backgroundColor: BG_GRAY }}>
          <div className="max-w-[1200px] mx-auto px-6 lg:px-14 py-14">
            <div className="max-w-3xl">
              ...
              >
                Чекап для чоловіків до 30 років – що перевіряти і де пройти в Харкові
              </h1>
              <p className="text-lg text-gray-700 leading-relaxed mt-2">Сторінка для чоловіків до 30 років без скарг, які хочуть зрозуміти, що варто перевірити зараз. Спочатку перелік за клінічними настановами, потім готова програма клініки в Харкові.</p>
            </div>
          </div>
        </section>
        ...
        <Section bg={BG_WHITE} eyebrow="За клінічними настановами">
          ...
          <p className={P}>Перше вимірювання холестерину роблять у 20 років. Якщо показники в нормі, його повторюють раз на 4–6 років<S n={[1]} />.</p>
          <h3 className="text-lg font-semibold text-[#0b1a24] mt-8">Простата: аналіз PSA</h3>
          <p className={P}>Рутинно аналіз PSA в цьому віці не пропонують: USPSTF говорить про нього для чоловіків 55–69 років<S n={[2]} />, урологи Mayo Clinic – з 50 років<S n={[1]} />. І тоді це спільне рішення з лікарем, а не автоматичний аналіз.</p>
        ...
        <Section bg={BG_GRAY} eyebrow="Ваша історія">
          ...
          <p className={P}>Якщо індекс маси тіла 25 або більше, з 35 років додають скринінг переддіабету і діабету 2 типу раз на 3 роки<S n={[3]} />.</p>
        </Section>
          ...
          <section style={{ backgroundColor: BG_WHITE, borderTop: BORDER }}>
            <div className="max-w-[1200px] mx-auto px-6 lg:px-14 py-10">
              <div className="max-w-3xl text-[14px] text-gray-600 leading-relaxed">
                <p>
                  Чекап для чоловіків до 30 років у Харкові можна пройти в {clinic.name}: {branches.length}{' '}
