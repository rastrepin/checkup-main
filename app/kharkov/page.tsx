...
export const metadata: Metadata = {
  title: 'Чекап организма в Харькове от 7 722 грн — комплексное обследование',
...
const FAQ_ITEMS = [
  { q: 'Сколько стоит чекап организма в Харькове?', a: 'Стоимость зависит от программы. Мужской профилактический — от 7 722 грн, женский профилактический — от 11 724 грн. Программы после 40 лет включают расширенную диагностику и стоят от 14 634 грн (женщины) и от 15 386 грн (мужчины). Цена включает все анализы, УЗИ и консультации специалистов.' },
  { q: 'Сколько времени занимает обследование?', a: 'Обследование проходит за 2 визита. Первый визит — 2-3 часа утром натощак (анализы, УЗИ, ЭКГ, консультации). Второй визит — через 5-7 дней, до 1 часа (терапевт с результатами и рекомендациями). Второй визит возможен онлайн.' },
  { q: 'Как подготовиться к чекапу?', a: 'За 24 часа до обследования не употребляйте алкоголь. Приходите натощак (последний приём пищи за 8-12 часов). Для женщин рекомендуется планировать визит на 5-12 день менструального цикла. Подробная инструкция по подготовке придёт после записи.' },
  { q: 'Где находятся клиники?', a: 'Обследования проводятся в сети «ОН Клиник Харьков» — три филиала у станций метро: ул. Ярослава Мудрого, 30а (М Пушкинская), пр. Героев Харькова, 257 (М Дворец Спорта), ул. Молочная, 48 (М Гагарина). Программы доступны во всех филиалах.' },
  { q: 'Можно ли пройти чекап в выходной день?', a: 'Да. Все три филиала работают в субботу (с 8:00-9:00). Филиал на Героев Харькова и Молочной также работает в воскресенье. Точный график уточняйте при записи.' },
];
    ...
    <>
      <main className="max-w-2xl mx-auto px-4 pb-24">
        ...
        <section className="mb-10">
          ...
          <a href="#quiz" className="inline-flex items-center px-6 py-3.5 rounded-[10px] bg-[#005485] text-white font-semibold text-[15px] hover:bg-[#004470] transition-colors">
            Подобрать программу
          </a>
        </section>

        <section id="quiz" className="mb-14 scroll-mt-4">
          <QuizWrapper clinicSlug={CLINIC_SLUG} city="kharkiv" locale="ru" sourcePage="/kharkov" branches={safeBranches} />
        </section>
        ...
        <section className="mb-14">
          ...
          <p className="text-[14px] text-gray-600 leading-relaxed mb-5">
            Медицинский центр «ОН Клиник Харьков» — сеть из трёх многопрофильных клиник международной сети ON Clinic, работающей в Украине с 2007 года. Собственная лаборатория «ОН Лаб», современное диагностическое оборудование и команда более 50 специалистов в трёх филиалах города.
          </p>
          <div className="space-y-3 mb-5">
            {safeBranches.map(branch => (
              <div key={branch.id} className="bg-white border border-gray-200 rounded-[10px] p-4">
                ...
                <div className="text-xs text-gray-500 mb-0.5">{branch.address_ru}{branch.metro_ru ? ` · ${branch.metro_ru}` : ''}</div>
                <div className="text-xs text-gray-400">Пн-Пт: {branch.schedule.mon_fri} · Сб: {branch.schedule.sat}</div>
              </div>
            ))}
          </div>
          <a href={SUBDOMAIN} className="text-sm font-semibold text-[#005485] hover:underline" target="_blank" rel="noopener noreferrer">Подробнее о клинике →</a>
        </section>

        <section className="mb-14 text-[13px] text-gray-500 leading-relaxed border-t border-gray-100 pt-8">
          <p className="mb-2">Комплексное обследование организма (чекап) в Харькове доступно в сети медицинских центров «ОН Клиник Харьков» по трём адресам: ул. Ярослава Мудрого, 30а (район Пушкинской), пр. Героев Харькова, 257 (район Дворца Спорта) и ул. Молочная, 48 (район Левады). Все филиалы расположены рядом со станциями метро.</p>
          <p className="mb-2">Стоимость комплексного обследования в Харькове — от 7 722 грн (мужской профилактический) до 15 386 грн (мужской после 40). Женские программы — от 11 724 грн. Специализированные программы — от 10 263 грн.</p>
          <p>Сервис check-up.in.ua организовывает запись на комплексное обследование в Харькове с 2019 года. Консультант свяжется с вами в течение рабочего дня.</p>
        </section>
        ...
        <section className="text-center py-10 border-t border-gray-100">
          ...
          <a href="#quiz" className="inline-flex items-center px-8 py-4 rounded-[10px] bg-[#005485] text-white font-semibold text-[15px] hover:bg-[#004470] transition-colors">Подобрать программу</a>
        </section>
      </main>
    </>
  );
